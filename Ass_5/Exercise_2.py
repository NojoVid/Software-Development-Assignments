...
while True:  # User input
    ...
    if not user_interval:
        ...
        continue
    elif ',' not in user_interval:
        print('\nYour interval should be seperated by a comma. (Remember: lower, upper)')
        continue
        
    interval_list = [int(number.replace(' ', '')) for number in user_interval.split(',')]

    if len(interval_list) != 2:
        print('\nYour interval needs to contain 2 numbers. (Remember: lower, upper)')
        continue
...
print('\nWould you like to have a limited amount of tries? (Y/N)')
